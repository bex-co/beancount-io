...
Also ask up front for each source account's **current balance** (from the old app or the real bank) — opening balances depend on it (see mapping.md).

### 2. Scaffold

If the working directory has no ledger, scaffold one via the **beancount-init** skill's flow (main.bean + Fava + uv + Makefile) — do not duplicate that logic here. If a populated ledger already exists, stop: this skill targets fresh starts; offer `beancount-import` instead.
...
- Every entry carries `import-id` metadata per the `beancount-import` convention (`references/dedup.md` there): native row ID if the export has one, else `<source>:sha256:<16-hex>` with the same normalization. A later `beancount-import` of the same account then dedups against migrated history automatically.
...
- **Row count**: source rows = converted transactions + 2×(transfer pairs merged) + skipped rows (each listed with a reason).
...
- Don't write anything outside the fresh ledger repo, and nothing before the mapping confirm.
