...
- `authz/model.fga` models the **relationship ceiling** of these semantics (owner/collaborator/public → rank) in the OpenFGA modeling language, with behavioral assertions in `authz/model.test.fga.yaml`; the credential ceiling (scopes, session exemption, ledger pin) deliberately stays in `identity.ts`, and `authorizeLedger` composes the two. Nothing evaluates the model at runtime — it is documentation-as-code — but `.github/workflows/ci-authz-model.yml` validates and tests it, so a semantic change to relationship resolution (`ledger-access-check.ts`, `authorizeRank`) must update the model in the same PR. See `authz/README.md`; the boundary and strategy (why no engine yet, the triggers that would change that, and the pre-decided engine choice) are **ADR 0010** (`../../docs/adrs/ADR010-backend-v2-authz-model.md`).
