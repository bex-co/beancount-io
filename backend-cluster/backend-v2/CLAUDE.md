...
- `authz/model.fga` models the **durable relationship ceiling** (exact-self users plus ledger owner/collaborator/public relationships) in the OpenFGA language, with behavioral assertions in `authz/model.test.fga.yaml`. Credentials and request context stay outside FGA and never become contextual tuples. No OpenFGA runtime is deployed. Protected user profile/lifecycle/API-key-management actions execute through the small TypeScript PDP in `src/server/api/authorization/`; Account/API-key workflows call it once before domain work. Exact-self comes from the resolved stable user ID, and API-key ownership is read from the current row without a tuple copy or cross-request cache. `authorizeLedger` remains unchanged. A semantic relationship change must update the model in the same PR. See `authz/README.md` and **ADR 0010** (`../../docs/adrs/ADR010-backend-v2-authz-model.md`).

### API keys

`src/features/apikeys/` issues durable `bcio_` credentials for CI, cron, CLI, and agents (ADR 0006 D6). GraphQL, REST, and MCP all call `ApiKeyWorkflow`, which makes one centralized authorization decision before the service. The PDP owns the existing admin credential ceiling, exact-self/key-owner relationship, and no-self-replication rule. The service then enforces the remaining domain constraints identically on all three surfaces:
