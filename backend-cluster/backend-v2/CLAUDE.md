...
- `authz/model.fga` models the **durable relationship ceiling** (exact-self users plus ledger owner/collaborator/public relationships) in the OpenFGA language, with behavioral assertions in `authz/model.test.fga.yaml`. Credentials, scopes, Stripe identifiers, temporary-object keys, and request context stay outside FGA and never become contextual tuples. No OpenFGA runtime is deployed. Protected user profile/lifecycle/API-key-management/billing/social/ledger-control-plane, temporary-asset, assisted-ingestion, and AI actions execute through the small TypeScript PDP in `src/server/api/authorization/`; authenticated single-ledger `read`/`write`/`admin` calls use the same contract through `authorizeLedger`. Protected application-service and workflow boundaries call it before domain work, with no raw user-ID bypass, and composite actions declare every capability family once in the catalog. Public profile/follower/following/starred-list discovery, anonymous public-ledger reads, and the static tier-quota catalog remain explicit exceptions. Exact-self comes from the resolved stable user ID, API-key ownership from the current row, Stripe ownership from its payment-domain invariant, ledger relationships from current Gitea/Fava facts, and temporary ownership from `tmp/{userId}/...`, all without a tuple copy, decision memo, or cross-request cache. A semantic relationship change must update the model in the same PR. See `authz/README.md` and **ADR 0010** (`../../docs/adrs/ADR010-backend-v2-authz-model.md`).
