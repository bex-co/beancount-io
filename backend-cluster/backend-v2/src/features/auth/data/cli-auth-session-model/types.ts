 ...
 * consent screen can show *something a person can recognize* ("bea 0.4 on
 * tian-mbp") instead of an anonymous "a CLI wants access", which is the prompt
 * a phished user has no way to refuse. The UI labels it as reported by the
 * device; nothing here is ever an authorization input.
 */
export interface CliAuthClientInfo {
  /** Client name as reported, e.g. `bea`. */
  name: string;
  version?: string;
