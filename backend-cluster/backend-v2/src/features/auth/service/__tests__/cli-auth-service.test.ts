...
describe("CliAuthService", () => {
  ...
  describe("authorizeSession", () => {
    it("issues a JWT and stores it on a pending session", async () => {
      ...
      await service.authorizeSession("cli-session-1", "user-1");
    ...
    it("throws when the session does not exist", async () => {
      ...
      await expect(
        service.authorizeSession("missing", "user-1"),
    ...
    it("throws when the session is no longer pending", async () => {
      ...
      await expect(
        service.authorizeSession("cli-session-1", "user-1"),
  ...
  describe("denySession", () => {
    it("denies a pending session", async () => {
      ...
      await service.denySession("cli-session-1");
    ...
    it("throws when the session is not pending", async () => {
      ...
      await expect(service.denySession("cli-session-1")).rejects.toThrow(
        "already been used",
      );
