...
/** Exact-self evaluator used by isolated unit tests with no database layer. */
export class ExactSelfRelationshipEvaluator implements IRelationshipEvaluator {
  public async check(input: RelationshipCheck): Promise<boolean> {
    const resource = parseAuthorizationResource(input.object);
    return Boolean(
      resource?.type === "user" &&
      USER_RELATIONSHIP_SET.has(input.relation) &&
      input.user === userResource(resource.id),
    );
  }
}
