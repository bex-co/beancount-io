...
describe("str utilities", () => {
  ...
  describe("createLedgerId and parseLedgerId", () => {
    ...
    it("should reject additional path separators", () => {
      expect(() => parseLedgerId("owner/my/ledger")).toThrow(
    ...
    it.each([
      ...
      "owner/%2e%2e",
      "owner\\admin/ledger",
      "owner/ledger%2Fother",
      "owner/ledger?ref=main",
    ])("should reject unsafe owner/repo path input %s", (value) => {
      expect(() => parseLedgerId(value)).toThrow("Invalid ledger ID");
    });
