#!/usr/bin/env bash
#
# One-time provisioning after `docker compose up -d --build` (up.sh calls this):
...
#
# Safe to re-run at any time. Adapted from deploy/docker-mac's
# post-docker-compose-up-build.sh for this compose project and its ports.
#
set -euo pipefail
...
if curl -sf http://localhost:42612 > /dev/null 2>&1; then
...
SERVICES=("devdns" "postgres" "postgres-backend" "redis" "gitea" "ledger" "backend-v2" "dashboard")
HEALTHY=0
TOTAL=${#SERVICES[@]}
...
if [ "$HEALTHY" -eq "$TOTAL" ]; then
    echo "🎉 Compose services are up. Dashboard: http://localhost:42610"
else
