...
**Resolution (2026-09-08):** the direct `ANTHROPIC_API_KEY` was deployed and verified end-to-end against production — a real receipt fixture parsed in ~10 s with account recommendations populated. Receipt parsing and every other Path B consumer are restored. Two standing realities until their follow-ups land: the fallback chain is **effectively single-legged** (the OpenAI fallback still fails strict `response_format` validation, so an Anthropic outage takes Path B down), and the gateway's array-content defect is **filed on the vendor's own board** — once fixed upstream, D3's "required" relaxes back to "recommended (independent failure domains)". One earlier inference withdrawn (2026-09-09): the OpenAI attempt reached OpenAI's schema validator not because the gateway's OpenAI route tolerated the array-content body, but because production had a direct `OPENAI_API_KEY` set all along — the fallback leg never touched the gateway. The outage logs therefore say nothing about BlockEden's OpenAI route either way.
...
## Amendment — 2026-09-09: reliability hardening (w2/m30) — most follow-ups closed

Shipped as the `.pm` milestone w2/m30. Each item below was a Follow-up above:

1. **`parseFile` can no longer crash the process (D6).** `parseReceipt`'s concurrent extraction+accounts fetch uses `Promise.allSettled` so a fast extraction failure can't leave a sibling rejection to escape as an unhandled rejection. A child-process gate (`upstream-survival.test.ts`) runs the real AI SDK against four hostile upstreams (auth reject, socket destroyed mid-body, garbage body, connection reset) under `--unhandled-rejections=strict` and asserts every failure surfaces as a thrown error, process alive.
2. **The OpenAI fallback is real, and the dialect is CI-enforced.** The receipt schema's `date` is now required + nullable (OpenAI strict `response_format` rejects bare-optional). A static gate (`structured-output-strict-compat.test.ts`) walks every `Output.object` schema in the feature and fails on any strict-mode violation; it also asserts the set of `Output.object` call sites is exactly the four registered. The child-process harness additionally proves the fallback leg end-to-end: Anthropic 401 → OpenAI Responses API serves a valid structured completion → parsed. The chain is no longer single-legged.
3. **Models are config, not code.** `LLM_MODEL` / `LLM_FALLBACK_MODEL` (deliberately not `ANTHROPIC_MODEL` — that belongs to the Path A sandbox) select the provider models, defaulting to the alias `claude-sonnet-4-5` / `gpt-4o`. The dead `OPENAI_MODEL` env var is deleted from every deploy target.
4. **Mobile shows the real cause (D5, client side).** `use-receipt-workflow.ts` classifies the parse leg from the thrown `ApolloError`'s `graphQLErrors[].extensions.code` (`RESOURCE_LIMIT_REACHED` → quota, else parse), the dead `result.errors` branch is gone, and every leg reports to Sentry via a thin `captureError` seam.
5. **Path B is probed (D6).** A synthetic probe fires once at boot (fire-and-forget) and hourly (`llm-probe` scheduler job): unconfigured logs one quiet info line and makes no traffic; configured-but-dead logs a loud warning within the first minute — the state that let the placeholder key hide in production.

Remaining open: the deprecated AI-SDK `"image"` content part (migrate to a `file` part with `mediaType` — a runtime deprecation warning, not a failure), and the **Haiku downgrade evaluation** (w2/m30/t006), which needs an operator-supplied `ANTHROPIC_API_KEY` to run the eval suite and is the one milestone task not closeable in the implementation environment.
