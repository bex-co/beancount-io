...
- **2026-09-09 — D2 exception: four discovery reads become tools (w2/m27).** The MCP field audit ran three Claude Code agent sessions (11 / 21 / 15 turns for three ordinary tasks) against the docker-mac stack: all three reported "no list-ledgers tool", "no errors/validation tool", and "resources returned empty". D2 put reads on resources because tools compete for selection attention — but in clients that never enumerate resource templates, the trade-off inverts for the handful of reads that precede everything else. The exception rule: a read becomes a tool when agent runs show it is the first thing an agent looks for and clients do not surface the resource. Four tools, each wrapping the same service call as its resource twin (D5: one list, two adapters): `listLedgers` (catalog/ledgers; pinned credentials return their one ledger), `checkLedger` (errors + entries-count + latest-commit), `getLedgerContext` (attributes, open accounts, currencies, payees, years, source files; counts alongside truncated lists), and `getEntryContext` (entry-context/{hash}). The resource twins stay; no other read is promoted.
- **2026-09-09 — compat-only shims leave the agent surface; `tools/list` measured, not wished (w2/m27).** The four GraphQL/REST compatibility shims (`addLegacyEntries`, the three legacy resources) are `mcpExempt: compatOnly`: agents must never choose the legacy spelling when the canonical verb serves the same capability, while older clients keep their REST twins. The three key-management verbs fold into one grouped `manageApiKeys` dispatcher. On size: the milestone aimed `tools/list` at 25 KB, but the wire measurement says output schemas alone are ~29 KB (D8 requires them published) and the nine directive shapes behind `addLedgerEntries` are the shared REST contract — the floor with every output schema deleted is still ~33 KB. The suite therefore pins the achieved ~60 KB (25 tools, shims gone, descriptions trimmed) as the regression gate instead of the unreachable 25 KB.
