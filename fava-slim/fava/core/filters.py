...
from ..util.date import DateRange, parse_date
...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from ..core.fava_options import FavaOptions
...
class FilterSyntaxLexer:
    ...
    def EQ_OP(self, token: str, value: str) -> tuple[str, str]:
        return token, value

    def CMP_OP(self, token: str, value: str) -> tuple[str, str]:
        return token, value

    def NUMBER(self, token: str, value: str) -> tuple[str, Decimal]:
...
class FilterSyntaxParser:
    ...
    def p_error(self, _: object | None) -> None:
    ...
    def p_simple_expr_key(self, p: list[Any]) -> None:
        ...
        match: Match | MatchAmount = Match(value) if op == ":" else MatchAmount(op, value)
    ...
    def p_simple_expr_units(self, p: list[Any]) -> None:
        ...
        def _range(entry: Directive) -> bool:
            return any(match(posting.units) for posting in getattr(entry, "postings", []))
...
class AccountFilter(EntryFilter):
    ...
    def apply(self, entries: Sequence[Directive]) -> Sequence[Directive]:
        ...
        if not value:
            return entries
        match = self._match
        return [entry for entry in entries if any(account.has_component(name, value) or match(name) for name in get_entry_accounts(entry))]
