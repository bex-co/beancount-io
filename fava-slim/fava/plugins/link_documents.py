...
def link_documents(
    ...
    for index, entry in enumerate(entries):
        disk_docs = [value for key, value in entry.meta.items() if key.startswith("document") and isinstance(value, str)]
        ...
        for disk_doc in disk_docs:
            documents = [j for j, document in by_basename[disk_doc] if document.account in entry_accounts]
            ...
            if not documents:
                errors.append(
                    DocumentError(entry.meta, f"Document not found: '{disk_doc}'", entry),
                )
                continue
