...
from ..beans.abc import Balance, Close
from ..beans.flags import FLAG_UNREALIZED
from ..beans.funcs import hash_entry
from ..core.conversion import units
from ..core.group_entries import TransactionPosting, group_entries_by_account
from ..core.tree import Tree
from ..util.date import local_today
...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from ..beans.abc import Directive, Meta
...
def uptodate_status(
    ...
    for txn_posting in reversed(txn_postings):
        ...
        if isinstance(txn_posting, TransactionPosting) and txn_posting.transaction.flag != FLAG_UNREALIZED:
            return "yellow"
    return None
...
class AccountsModule(dict[str, AccountData]):
    ...
    def all_balance_directives(self) -> str:
        ...
        return "".join(account_details.balance_string for account_details in self.values() if account_details.balance_string)
