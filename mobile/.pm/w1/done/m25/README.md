...
**Worker:** worker1 **Goal:** When account-picker fuzzy search finds nothing, the user can create the typed account on the spot — the YNAB "New Category" pattern — landing in the existing open-account flow prefilled, and returning with the new account already selected in the form they came from. **Status:** **done** 2026-08-18 — the picker's zero-results state creates the typed account and hands it back to the calling form; the success-path walk needs a real ledger write and is split out to inbox `030`
...
| t003 | Selection handoff: a successful open selects the account in the caller | 45m | t001, t002 | — **DONE** |
