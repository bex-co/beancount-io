...
async function api(method, url, body) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`${method} ${url} -> ${res.status}: ${detail?.title || ""} ${detail?.detail || text.slice(0, 200)}`);
  }
  return json;
}
...
(async () => {
  ...
  const existing = await api("GET", `/appScreenshotSets/${SET_ID}/appScreenshots`);
  ...
  for (const name of files) {
    ...
    ids.push(id);
    console.log(`uploaded ${name} (${(buf.length / 1024).toFixed(0)} KB) -> ${id}`);
  }

  // 3. Pin display order explicitly rather than relying on creation order.
  await api("PATCH", `/appScreenshotSets/${SET_ID}/relationships/appScreenshots`, {
