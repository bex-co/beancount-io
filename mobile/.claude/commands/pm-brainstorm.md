...
3. **Consult the platform repo (private, read-only).** The beancount.io platform lives in a **private** sibling checkout at `../beancount-io` relative to this repo's root (`../../beancount-io` from `mobile/`). Use it to ground proposals in what the platform already supports, instead of guessing from the mobile app's generated GraphQL schema alone:
   ...
   **Privacy guard (hard rule):** this monorepo is open source; `beancount-io` is not, and everything `/pm` materializes into `.pm/` is public. Reference private material by path and operation name only — never copy proprietary code, internal hostnames/URLs, credentials, infra topology (`backend-cluster/_infra*`), or business metrics into proposals or any file in this repo. If the sibling checkout is absent, proceed without it.
