...
**Worker:** worker1 **Goal:** a newcomer who taps **Sign Up** on the mobile welcome screen is taken straight to a registration form in the system browser and returns to the app signed into the account they just created; the welcome screen stops explaining browsers **Status:** in progress (t001–t008 done; t009 closeout blocked on shipping backend-v2 + dashboard, then a hosted end-to-end run — fresh sign-up needs the user's go-ahead to create an account)
