...
**Worker:** worker1 **Goal:** the one-line `include "https://beancount.io/prices/BTC-USD"` that PRFAQ002 promises works in the CLI too — in every load path, offline-safe, with inspectable freshness and a portable escape hatch **Status:** todo

## Tasks (in order)

| id | title | est | depends_on |
| --- | --- | --- | --- |
| t001 | Mirror the ADR 015 URL policy, bounded fetch, and price-only validation in the CLI | 60m | — |
| t002 | Cache feeds locally with an immutable revision and a mutable head | 60m | t001 |
