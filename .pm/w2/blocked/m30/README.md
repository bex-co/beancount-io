...
**Worker:** worker2 **Goal:** every ADR 0011 follow-up closed — the AI ingestion path cannot crash the process, the vendor fallback genuinely works, models are config not code, failures surface with their real cause on every client, and a dead credential is detected before a user finds it **Status:** blocked (t001–t005, t007–t009, t011 done; t006 blocked on an operator ANTHROPIC_API_KEY; t010 pending t006)
...
**Blocked 2026-09-23.** Every open task is stuck on one external credential. t006 (Haiku-vs-Sonnet extraction eval) needs two real promptfoo passes against the Anthropic API, and no `ANTHROPIC_API_KEY` is available in the implementation environment (re-checked 2026-09-23: none exported, and `backend-cluster/backend-v2/` holds only `.env.example`). t010 Closeout depends on t006, and the definition of done's last bullet (a recorded eval decision) cannot hold without it.

- **Baseline:** all code work shipped (t001–t005, t007–t009, t011). The eval provider already honors `LLM_MODEL`, so no code change is needed to run the passes.
- **Unblock:** an operator exports a working `ANTHROPIC_API_KEY` in the worker environment (or runs the two passes and supplies the outputs), then `/pm unblock w2/m30` and finish t006 → t010.
- **Who can clear it:** the repository owner/operator holding the Anthropic credential.
