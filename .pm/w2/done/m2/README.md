...
**Worker:** worker2 **Goal:** from a sample file of an unsupported bank, an agent produces a working beangulp importer that passes its own test harness — and repairs an existing importer when the bank's format drifts **Status:** done

## Tasks (in order)

| id   | title                                                       | est | depends_on |
| ---- | ----------------------------------------------------------- | --- | ---------- |
| t001 | SKILL.md skeleton: triggers/skip, relationship to import UX — **DONE** | 30m | —          |
| t002 | Distilled beangulp v3 reference (API + test harness) — **DONE** | 1h  | t001       |
| t003 | Authoring workflow: sample-driven draft → test → iterate — **DONE** | 1h  | t002       |
| t004 | Evals: fixture bank files + expected extractions — **DONE** | 1h  | t003       |
