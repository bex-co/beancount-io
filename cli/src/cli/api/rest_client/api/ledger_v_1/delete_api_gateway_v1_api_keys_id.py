...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "delete",
        "url": "/api-gateway/v1/api-keys/{id}".format(
...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> ApiKey | V1Error | None:
...
def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[ApiKey | V1Error]:
    return Response(
...
) -> Response[ApiKey | V1Error]:
    """Revoke an API key

     Stops the key working from the next request onward. Revoking twice is not an error, and a key id you
    do not own reads as not found.

    Args:
        id (str): The key's id (`akey_…`), not the key itself Example: akey_7wXzK9mNpQrSt2VxYaBc.
