...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api-gateway/v1/ledgers/{owner}/{name}/postings-per-account".format(
...
def sync_detailed(
    ...
    *,
    ...
    """Count postings per account
     ...
     Posting counts for the filtered Statistics report stream. An account filter selects matching entries
    and counts every posting on those entries, including counterpart accounts. Time filters use the same
    clamped report stream as entries-count.
