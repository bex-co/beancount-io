...
from ...models.bql_query import BqlQuery
...
def _get_kwargs(
    ...
    name: str,
    *,
    body: BqlQuery | Unset = UNSET,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api-gateway/v1/ledgers/{owner}/{name}/query".format(
...
def sync_detailed(
    ...
    *,
    ...
    """Run a BQL query
     ...
     Runs a Beancount Query Language statement against the ledger. `Accept: application/json` returns a
    typed table; `Accept: text/plain` returns the shell's own text rendering.

    Args:
        owner (str): Ledger owner's username Example: alice.
        name (str): Ledger (repository) name Example: main-ledger.
        body (BqlQuery | Unset): A BQL query to run against the ledger
