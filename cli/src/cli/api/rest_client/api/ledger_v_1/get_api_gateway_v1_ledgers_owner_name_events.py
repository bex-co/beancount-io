...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api-gateway/v1/ledgers/{owner}/{name}/events".format(
...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> V1Error | list[Any] | None:
...
def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[V1Error | list[Any]]:
    return Response(
...
) -> Response[V1Error | list[Any]]:
    """List event directives

     Every `event` directive in the ledger, in date order.
