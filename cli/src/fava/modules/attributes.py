...
from ..util.date import END_OF_YEAR
from ..util.ranking import ExponentialDecayRanker


if TYPE_CHECKING:  # pragma: no cover
    from collections.abc import Sequence

    from ..beans.abc import Directive, Transaction
...
def get_active_years(
    ...
    for entry in entries:
        date = entry.date
        year = entry.date.year + 1 if date.month > month or (date.month == month and date.day > day) else entry.date.year
        if year != prev_year:
            prev_year = year
            years.append(year)
