...
- Release: `.github/workflows/deploy.yml` (workflow name `Release (mobile)`) runs on every `mobile/**` push to `main`: verifies checks, ships an OTA update, and — when `mobile/package.json`'s version has no `mobile-v<version>` git tag yet — runs the Expo EAS build/submit, then pushes the tag and a GitHub Release. Tag-after-success makes failed releases retry automatically on the next push.
