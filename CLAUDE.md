...
- Release (cli): `.github/workflows/release-cli.yml` (workflow name `Release (cli)`) runs on `cli-v<version>` tags. It validates the tag against `cli/pyproject.toml`, runs `make check-all`, publishes the sdist and wheel to PyPI through trusted publishing, creates the GitHub Release, and pushes a rendered `Formula/bea.rb` to `bex-co/homebrew-tap` (skipped when `BEA_TAP_PUSH_KEY` is unset). `workflow_dispatch` with `test` rehearses the whole thing against TestPyPI. See `cli/README.md` for the tagging procedure.
